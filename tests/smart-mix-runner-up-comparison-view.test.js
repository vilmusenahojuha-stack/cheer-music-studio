...
const explanation={
  comparison:{
    selected:{
      ...
      reasons:[{code:'purpose',label:'sopii osuuden tarkoitukseen'}]
    },
    runnerUp:{
      sourceName:'B.wav',
      trackId:'track-b',
      ...
      reasons:[{code:'boundary',label:'osuu hyvään fraasirajaan'}]
    },
    scoreMargin:.07,
    sameSource:false,
    nonDestructive:true
...
const close=view.buildRunnerUpComparisonView({
  comparison:{
    selected:{sourceName:'A.wav',startEight:1,endEight:4,score:.88},
    runnerUp:{sourceName:'A.wav',startEight:9,endEight:12,score:.86},
    scoreMargin:.02,
    sameSource:true
  }
});
...
const tied=view.buildRunnerUpComparisonView({
  comparison:{
    selected:{sourceName:'A.wav',score:.80},
    runnerUp:{sourceName:'B.wav',score:.80},
    scoreMargin:0,
    sameSource:false
  }
});
...
const malformed=view.buildRunnerUpComparisonView({
  comparison:{
    selected:{sourceName:'A.wav',startEight:-3,endEight:2,score:4},
    runnerUp:{sourceName:'B.wav',score:-2},
    scoreMargin:9,
    sameSource:false
  }
});
...
console.log('smart-mix runner-up comparison view tests passed');
