...
function makeSequentialProject(){return{duration:12,targetBpm:147,tracks:[{name:'big-boom',url:'blob:big',duration:8},{name:'menestys',url:'blob:men',duration:8}],trackAnalysis:{'big-boom':{bpm:142},menestys:{bpm:104.3}},mixSettings:{autoDuck:false},audioTimeline:{zoom:1,snap:'beat',clips:[{id:'big',type:'music',sourceName:'big-boom',start:0,duration:7,sourceOffset:0,volume:1,fadeIn:0,fadeOut:0},{id:'men',type:'music',sourceName:'menestys',start:7,duration:1,sourceOffset:0,volume:1,fadeIn:0,fadeOut:0}]}}}
...
(async()=>{
  ...
  vm.createContext(previewSandbox);vm.runInContext(fs.readFileSync(require.resolve('../timeline-audio-engine.js'),'utf8'),previewSandbox,{filename:'timeline-audio-engine.js'});const engine=previewWindow.cheerTimelineAudioEngine;await engine.startAt(0);
  ...
  // Reproduce the real Big Boom -> Menestys case: two different pitch-preserving ratios,
  // with the second clip outside the initial look-ahead window.
  engine.stop();engine.clearBuffers();Object.assign(state,makeSequentialProject());const ac=FakeAudioContext.last,before=ac.starts.length,sampleTolerance=1/8000+1e-9;
  try{
    await engine.startAt(0);let seq=ac.starts.slice(before);assert.equal(seq.length,1,'only Big Boom should be scheduled initially');const bigRate=147/142,menRate=147/104.3;assert.equal(seq[0].rate,1);assert.ok(Math.abs(seq[0].buffer.duration-(8/bigRate))<=sampleTolerance,'Big Boom stretched duration must be accurate within one sample');const info=engine.getClockInfo();ac.currentTime=info.contextAnchor+2;engine.runSchedulerNow();seq=ac.starts.slice(before);assert.equal(seq.length,2,'Menestys must be scheduled when it enters look-ahead');assert.equal(seq[1].rate,1,'Menestys must also keep playbackRate at 1');assert.ok(Math.abs(seq[1].when-(info.contextAnchor+7))<1e-9,'Menestys must start at the exact timeline handoff');assert.ok(Math.abs(seq[1].buffer.duration-(8/menRate))<=sampleTolerance,'Menestys stretched duration must be accurate within one sample');
  }finally{engine.stop()}
  ...
  console.log('pitch-preserving integration: preview/export mapping and sequential 1.035x -> 1.409x handoff passed');
