...
const renderAt=exportJs.indexOf('CheerOfflineRenderer.renderProject'),measureAt=exportJs.indexOf('measureCompetitionMaster(rendered)'),readinessAt=exportJs.indexOf('evaluateCompetitionMasterReadiness(competition?.metrics)'),finalCheckAt=exportJs.indexOf('buildCompetitionMasterFinalCheck'),headroomAt=exportJs.indexOf('applyMasterHeadroom(rendered');
assert.ok(renderAt>=0&&measureAt>renderAt&&readinessAt>measureAt&&finalCheckAt>=0&&headroomAt>readinessAt,'competition metrics, readiness, preview and final check must evaluate the untouched post-voiceover render before headroom gain changes PCM');
...
console.log('lossless-export-contract: OfflineAudioContext -> post-voiceover metrics -> competition readiness -> advisory preview -> advisory final-check -> headroom -> 24-bit WAV path enforced');