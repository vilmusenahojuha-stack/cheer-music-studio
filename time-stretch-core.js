((root,factory)=>{const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.CheerTimeStretch=api})(typeof globalThis!=='undefined'?globalThis:this,()=>{
  ...
  function stretchChannels(channels,sampleRate,rate,options={}){
    ...
    const synthesisHop=Math.max(64,frame-overlap),analysisHop=synthesisHop*r;
    ...
    let synthPos=synthesisHop,prevInputPos=0;const maxStart=Math.max(0,inputLength-frame);
    while(synthPos<outputLength){
      const expected=prevInputPos+analysisHop;let best=Math.round(expected),bestScore=-Infinity;const lo=Math.max(0,Math.floor(expected-search)),hi=Math.min(maxStart,Math.ceil(expected+search));
      ...
      best=clamp(Math.round(best),0,maxStart);const remainingOut=outputLength-synthPos,remainingIn=inputLength-best,write=Math.min(frame,remainingOut,remainingIn),ov=Math.min(overlap,write);
      ...
      prevInputPos=best;synthPos+=synthesisHop;if(write<=0)break;
    }
    return out;
  }
