((root,factory)=>{const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.CheerMixDSP=api})(typeof globalThis!=='undefined'?globalThis:this,()=>{
  ...
  function transitionTiming(clip,transitionType,defaults={}){
    const c=clip||{},transition=c?.smartMix?.transitionOut||{};
    if(transition.type!==transitionType)return null;
    ...
    const countSeconds=dur/(eights*8),fade=clamp(num(c.fadeOut),0,dur);
    const countLength=num(transition.countLength)>0?num(transition.countLength):num(defaults.countLength,.25);
    const prep=clamp(countSeconds*countLength,fade,dur);
    if(!(prep>fade))return null;
    return{prepSeconds:prep,fadeSeconds:fade,countLength,floor:clamp(num(transition.anticipationFloor,defaults.floor),defaults.minFloor,defaults.maxFloor)};
  }
  ...
  function clipEnvelopeAt(clip,t){
    ...
    const remain=Math.max(0,dur-local),shape=transitionShape(c);
    if(shape&&remain<shape.prepSeconds){
      if(remain<=shape.fadeSeconds){
        const fadeProgress=shape.fadeSeconds>0?remain/shape.fadeSeconds:0;
        env=Math.min(env,shape.floor*clamp(fadeProgress,0,1));
      }else{
        const span=Math.max(.000001,shape.prepSeconds-shape.fadeSeconds);
        const progress=clamp((shape.prepSeconds-remain)/span,0,1);
        env=Math.min(env,1-(1-shape.floor)*progress);
      }
