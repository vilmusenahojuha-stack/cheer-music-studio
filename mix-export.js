(()=>{
  ...
  function progress(info){if(info.phase==='decode'){const pct=info.total?Math.round(info.done/info.total*45):5;setStatus(`Dekoodataan lossless-renderiä ${info.done}/${info.total}${info.name?` · ${info.name}`:''}…`,pct)}else if(info.phase==='render')setStatus(info.done?'Offline-render valmis, viimeistellään master-headroom…':'Renderöidään 48 kHz PCM offline…',info.done?88:55);else setStatus('Valmistellaan lossless-renderöintiä…',2)}
  async function exportWav(){
    ...
    try{
      ...
      const rendered=await window.CheerOfflineRenderer.renderProject(state,progress),before=analyzeBuffer(rendered);
      ...
      setStatus(`Valmis: ${safeName()}.wav · 48 kHz · 24-bit PCM · stereo · peak ${after.peakDb.toFixed(1)} dBFS · RMS ${after.rmsDb.toFixed(1)} dBFS${gainInfo}`,100);
  ...
  function init(){ensure();addUi();window.cheerMixExport={exportWav,validate,analyzeBuffer,applyMasterHeadroom}}
