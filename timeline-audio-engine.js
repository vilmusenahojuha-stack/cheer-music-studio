(()=>{
  ...
  let ctx=null,masterGain=null,running=false,starting=false,timelineAnchor=0,contextAnchor=0,generation=0;
  ...
  function rateFor(c){
    ...
    const bpm=Number(state.trackAnalysis?.[c.sourceName]?.bpm||state.trackAnalysis?.[c.sourceName]?.autoBpm),target=Number(state.targetBpm)||147;
  ...
  async function decodeTrack(t){
    ...
    const promise=(async()=>{
      ...
      bufferCache.set(key,buffer);decodePromises.delete(key);return buffer;
    })().catch(err=>{decodePromises.delete(key);throw err});
    decodePromises.set(key,promise);return promise;
  }
  async function preloadFor(playFrom){
    ensure();const tracks=new Map();
  ...
  function scheduleClipEnvelope(param,c,startTimeline,endTimeline,when){
    const base=clamp(Number.isFinite(Number(c.volume))?Number(c.volume):1,0,1),cStart=Number(c.start)||0,cEnd=cStart+Math.max(0,Number(c.duration)||0),fi=Math.max(0,Number(c.fadeIn)||0),fo=Math.max(0,Number(c.fadeOut)||0);
    param.cancelScheduledValues(when);param.setValueAtTime(Math.max(0.000001,clipEnvelopeAt(c,startTimeline)),when);
    if(fi>0){const fadeEnd=cStart+fi;if(fadeEnd>startTimeline&&fadeEnd<endTimeline)param.linearRampToValueAtTime(Math.max(0.000001,base),when+(fadeEnd-startTimeline))}
    if(fo>0){const fadeStart=cEnd-fo;if(fadeStart>startTimeline&&fadeStart<endTimeline){param.setValueAtTime(Math.max(0.000001,base),when+(fadeStart-startTimeline));param.linearRampToValueAtTime(0.000001,when+(cEnd-startTimeline))}}
  }
  function voiceWindows(){
    ...
    return state.audioTimeline.clips.filter(v=>v.type==='voice'&&(Number.isFinite(Number(v.volume))?Number(v.volume):1)>0).map(v=>{const s=Number(v.start)||0,d=Math.max(0,Number(v.duration)||0);return{start:s-attack,voiceStart:s,voiceEnd:s+d,end:s+d+release,attack,release,duck}});
  }
  function duckFactorAt(t){
    if(state.mixSettings.autoDuck===false)return 1;let f=1;
    for(const w of voiceWindows()){
      if(t<w.start||t>w.end)continue;
      let x=w.duck;
      if(t<w.voiceStart)x=1-(1-w.duck)*clamp((t-w.start)/Math.max(.0001,w.attack),0,1);
      else if(t>w.voiceEnd)x=w.duck+(1-w.duck)*clamp((t-w.voiceEnd)/Math.max(.0001,w.release),0,1);
      f=Math.min(f,x);
    }
    return f;
  }
  function scheduleDucking(param,c,startTimeline,endTimeline,when){
    param.cancelScheduledValues(when);param.setValueAtTime(c.type==='music'?duckFactorAt(startTimeline):1,when);
    if(c.type!=='music'||state.mixSettings.autoDuck===false)return;
    for(const w of voiceWindows()){
      if(w.end<=startTimeline||w.start>=endTimeline)continue;
      const points=[
        [Math.max(startTimeline,w.start),duckFactorAt(Math.max(startTimeline,w.start))],
        [Math.max(startTimeline,w.voiceStart),w.duck],
        [Math.min(endTimeline,w.voiceEnd),w.duck],
        [Math.min(endTimeline,w.end),duckFactorAt(Math.min(endTimeline,w.end))]
      ].filter((p,i,a)=>p[0]>=startTimeline&&p[0]<=endTimeline&&(i===0||Math.abs(p[0]-a[i-1][0])>.000001));
      for(let i=0;i<points.length;i++){
        const [t,v]=points[i],ct=when+(t-startTimeline);
        if(i===0)param.setValueAtTime(Math.max(0.000001,v),ct);else param.linearRampToValueAtTime(Math.max(0.000001,v),ct);
      }
    }
  }
  function duckFactor(now,c){return c?.type==='music'?duckFactorAt(now):1}

  function stopAllSources(){
    for(const item of [...activeSources]){
      try{item.source.stop()}catch{}
      try{item.source.disconnect()}catch{}
      try{item.clipGain.disconnect()}catch{}
      try{item.duckGain.disconnect()}catch{}
  ...
  async function scheduleAll(playFrom){
    ...
    const myGeneration=++generation;stopAllSources();timelineAnchor=Math.max(0,Number(playFrom)||0);contextAnchor=0;
    await preloadFor(timelineAnchor);if(myGeneration!==generation||!running)return false;
    const lead=Math.max(.10,(ac.baseLatency||0)+.05);contextAnchor=ac.currentTime+lead;
