(()=>{
  ...
  function playheadSec(){const left=parseFloat(q('#playhead')?.style.left||'0')||0;return Math.max(0,left/Math.max(.0001,pxPerSec()))}
  ...
  function duckFactor(now,c){
    ...
    const duck=Math.pow(10,clamp(Number(state.mixSettings.duckDb)||-7,-18,0)/20),attack=clamp(Number(state.mixSettings.duckAttack)||.08,.01,1),release=clamp(Number(state.mixSettings.duckRelease)||.18,.01,2);
    let factor=1;
    for(const v of state.audioTimeline.clips){
      if(v.type!=='voice'||(Number(v.volume)||1)<=0)continue;
      const s=Number(v.start)||0,e=s+Math.max(0,Number(v.duration)||0);
      if(now<s-attack||now>e+release)continue;
      let f=duck;
      if(now<s)f=1-(1-duck)*clamp((now-(s-attack))/attack,0,1);
      else if(now>e)f=duck+(1-duck)*clamp((now-e)/release,0,1);
      factor=Math.min(factor,f);
    }
    return factor;
  }
  function volumeFor(c,local,now){
    ...
    const remain=Math.max(0,(Number(c.duration)||0)-local);if(fo>0&&remain<fo)env=Math.min(env,remain/fo);
    return clamp(base*env*duckFactor(now,c),0,1);
  }
  function stopOne(id){const p=players.get(id);if(!p)return;try{p.audio.pause();p.audio.removeAttribute('src');p.audio.load()}catch{}players.delete(id)}
  function stopAll(){for(const id of [...players.keys()])stopOne(id)}
  function makePlayer(c,t,now){
    const audio=new Audio();audio.preload='auto';audio.src=t.url;audio.playsInline=true;
    try{audio.preservesPitch=true;audio.webkitPreservesPitch=true;audio.mozPreservesPitch=true}catch{}
    const rate=rateFor(c);audio.playbackRate=rate;const local=Math.max(0,now-c.start),desired=(Number(c.sourceOffset)||0)+local*rate;
    const seek=()=>{try{audio.currentTime=Math.max(0,desired)}catch{}};audio.readyState>=1?seek():audio.addEventListener('loadedmetadata',seek,{once:true});
    audio.volume=volumeFor(c,local,now);audio.play().catch(()=>{});const p={audio,rate,sourceName:c.sourceName};players.set(c.id,p);return p;
  }
  function sync(){
    if(!running)return;ensure();const now=playheadSec(),wanted=new Set();
    for(const c of state.audioTimeline.clips){
      const start=Number(c.start)||0,dur=Math.max(0,Number(c.duration)||0);if(now<start||now>=start+dur)continue;const t=trackFor(c);if(!t?.url)continue;
      wanted.add(c.id);const local=now-start,rate=rateFor(c),desired=(Number(c.sourceOffset)||0)+local*rate;let p=players.get(c.id);
      ...
      p.audio.volume=volumeFor(c,local,now);
    ...
    for(const id of [...players.keys()])if(!wanted.has(id))stopOne(id);raf=requestAnimationFrame(sync);
  }
  function start(){if(running)return;running=true;q('#audioPlayer')?.pause();sync()}
  ...
  function refreshInspector(){
    ...
    const rate=rateFor(c),a=state.trackAnalysis?.[c.sourceName],bpm=Number(a?.bpm||a?.autoBpm);q('#clipTempoInfo').textContent=c.type==='music'&&bpm?`${bpm.toFixed(1)} → ${state.targetBpm} BPM · ${rate.toFixed(3)}× · pitch säilytetään`:(c.type==='music'?'Tempo: alkuperäinen':'Nopeus 1.000×');
  }
  function init(){ensure();addInspector();const btn=q('#btnTimelinePlay');if(btn)new MutationObserver(followTransport).observe(btn,{childList:true,characterData:true,subtree:true});q('#timelineContent')?.addEventListener('pointerdown',()=>setTimeout(refreshInspector,0));window.addEventListener('pointerup',()=>setTimeout(refreshInspector,0));window.addEventListener('keydown',e=>{if(e.key==='Delete'||e.key==='Backspace')setTimeout(refreshInspector,0)});document.addEventListener('cheer-audio-restored',()=>{if(running){stop();setTimeout(()=>{if(transportPlaying())start()},20)}});window.addEventListener('beforeunload',stopAll);refreshInspector();followTransport();window.cheerTimelineAudioEngine={start,stop,rateFor,refreshInspector,autoCrossfade,duckFactor}}
