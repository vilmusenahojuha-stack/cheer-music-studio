(()=>{
  ...
  async function prepareStretchedBuffers(offline,project,buffers,onProgress=()=>{}){
    const ts=stretch(),hq=hqStretch(),prepared=new Map(),jobs=new Map();
    ...
    const items=[...jobs.entries()];if(items.length&&!hq?.stretchAudioBuffer&&!ts?.stretchAudioBuffer)throw new Error('Pitch-preserving time-stretch -ydin ei ole latautunut.');
    for(let i=0;i<items.length;i++){const[key,job]=items[i];onProgress({phase:'stretch',done:i,total:items.length,name:job.t.name});const rendered=hq?.stretchAudioBuffer?await hq.stretchAudioBuffer(offline,job.original,job.rate):ts.stretchAudioBuffer(offline,job.original,job.rate);prepared.set(key,rendered);onProgress({phase:'stretch',done:i+1,total:items.length,name:job.t.name})}
    return prepared;
  }
